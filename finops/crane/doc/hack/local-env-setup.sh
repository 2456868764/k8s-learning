...
kind create cluster --kubeconfig "${CRANE_KUBECONFIG}" --name "${CRANE_CLUSTER_NAME}"  --config=./crane_kind_config.yaml  --image kindest/node:v1.21.1
...
helm repo update
helm install crane -n crane-system --set craneAgent.enable=true --create-namespace ${CRANE_HELM_NAME}/crane
