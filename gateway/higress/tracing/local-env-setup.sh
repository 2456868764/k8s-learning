...
if [ "$IS_INSTALLED_CRD" = true ]; then
  ...
  helm install higress -n higress-system higress.io/higress --devel --create-namespace --render-subchart-notes  --set global.enableIstioAPI=true --set global.kind=true --set higress-console.o11y.enabled=true \
   --set higress-core.skywalking.enabled=true --set higress-core.skywalking.service.address=skywalking-oap-server.op-system.svc.cluster.local --set higress-core.skywalking.service.port=11800 \
   --set higress-controller.domain=console.higress.io --set higress-console.admin.password.value=admin
  echo "Step3: Installing Higress finished."
else
  echo "Step2: Installing Higress "
  helm repo add higress.io https://higress.io/helm-charts
  helm install higress -n higress-system higress.io/higress --devel --create-namespace --render-subchart-notes  --set global.enableIstioAPI=false --set global.kind=true --set higress-console.o11y.enabled=true  \
  --set higress-core.skywalking.enabled=true --set higress-core.skywalking.service.address=skywalking-oap-server.op-system.svc.cluster.local --set higress-core.skywalking.service.port=11800 \
  --set higress-controller.domain=console.higress.io --set higress-console.admin.password.value=admin
