...
kind create cluster --kubeconfig "${HIGRESS_KUBECONFIG}" --name "${HIGRESS_CLUSTER_NAME}"  --config=`pwd`/ingress_kind_config.yaml  --image kindest/node:v1.22.9
...
echo "    higress grafana url: http://console.higress.io:8080/prometheus"
